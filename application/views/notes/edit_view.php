	...
	<section id="content" class="wide-content">
		<div class="grid_6">
			<form action="" method="POST">
				...
				<input type="date" name="data" placeholder="2014-01-01" value="<?php if (isset($note['data'])) {
	echo $note['data'];
} ?>" required>
				<div class="label"><?php echo $this->lang->line('title'); ?></div>
				<input type="text" name="title" value="<?php if (isset($note['title'])) {
	echo $note['title'];
} ?>" required>
				<div class="label"><?php echo $this->lang->line('text'); ?></div>
				<input type="text" name="text" value="<?php if (isset($note['text'])) {
	echo $note['text'];
} ?>" required>
